...
function initEditor() {
	...
	for(var i = 0; i < buttons.length; i++) {
		if(buttons[i].id.indexOf(editorid + '_') != -1) {
			...
			if(buttons[i].id.substr(buttons[i].id.indexOf('_') + 1) == 'fullswitcher') {
				buttons[i].innerHTML = !editorisfull ? 'Toàn màn hình' : 'Đơn giản';
				buttons[i].onmouseover = function(e) {setEditorTip(editorisfull ? 'Chỉnh sửa đơn giản' : 'Chỉnh sửa toàn màn hình');};
	...
	if($(editorid + '_fullswitcher') && BROWSER.ie && BROWSER.ie < 7) {
		$(editorid + '_fullswitcher').onclick = function () {
			showDialog('Trình duyệt của bạn không hỗ trợ', 'notice', 'Nhắc nhở');
		};
...
function savedataTime() {
	if(!autosave) {
		$(editorid + '_svdsecond').innerHTML = '<a title="Ấn vào để bật tự động lưu" href="javascript:;" onclick="setAutosave()">Bật tự động lưu</a> ';
		return;
	}
	if(!savedatac) {
		savedatac = savedataInterval;
		...
		h = h < 10 ? '0' + h : h;
		m = m < 10 ? '0' + m : m;
		setEditorTip('Dữ liệu được ' + h + ':' + m + ' lưu');
	}
	$(editorid + '_svdsecond').innerHTML = '<a title="Ấn để tắt tự động lưu" href="javascript:;" onclick="setAutosave()">' + savedatac + ' giây lưu</a> ';
	savedatac -= 10;
}

function setAutosave() {
	autosave = !autosave;
	setEditorTip(autosave ? 'Đã bật lưu tự động' : 'Đã tắt lưu tự động');
...
function editorfull(op) {
	...
	$(editorid + '_fullswitcher').innerHTML = editorisfull ? 'Trở lại' : 'Toàn màn hình';
	initesbar();
}
...
function editorsimple() {
	if($(editorid + '_body').className == 'edt') {
		...
		v = '';
		$(editorid + '_simple').innerHTML = 'Đơn giản';
...
function checklength(theform) {
	...
	showDialog('Độ dài: ' + mb_strlen(message) + ' ký tự,' + (postmaxchars != 0 ? 'Hệ thống giới hạn: ' + postminchars + ', còn ' + postmaxchars + ' ký tự.' : ''), 'notice', 'Kiểm tra ký tự');
}
...
function discuzcode(cmd, arg) {
	...
	if(in_array(cmd, ['sml', 'inserthorizontalrule', 'url', 'quote', 'code', 'free', 'hide', 'aud', 'vid', 'fls', 'beginning', 'attach', 'image', 'pasteword', 'index', 'postbg', 'password']) || typeof EXTRAFUNC['showEditorMenu'][cmd] != 'undefined' || cmd == 'tbl' || in_array(cmd, ['fontname', 'fontsize', 'forecolor', 'backcolor']) && !arg) {
		...
		loadData();
		setEditorTip('Dữ liệu được khôi phục');
	} else if(cmd == 'svd') {
		saveData();
		setEditorTip('Đã lưu');
	} else if(cmd == 'chck') {
...
function showEditorMenu(tag, params) {
	...
	if(menu) {
		...
		switch(tag) {
			case 'url':
				str = 'Nhập URL:<br /><input type="text" id="' + ctrlid + '_param_1" style="width: 98%" value="" class="px" />'+
					(selection ? '' : '<br />Nhập URL:<br /><input type="text" id="' + ctrlid + '_param_2" style="width: 98%" value="" class="px" />');
				break;
			case 'forecolor':
			...
			case 'postbg':
				...
				break;
			case 'password':
				str = '<p class="pbn">Nhập mật khẩu bài viết: <input type="text" id="' + ctrlid + '_param_1" size="10" value="" class="px" /></p>';
				break;
			case 'code':
				if(wysiwyg) {
			...
			case 'free':
				...
				var lang = {'quote' : 'Nhập trích dẫn', 'code' : 'Nhập đoạn code', 'hide' : 'Nhập nội dung muốn ẩn', 'free' : 'Nếu bạn dùng chức năng bán chủ đề thì có thể dùng cái này cho việc mô tả'};
				str += lang[tag] + ':<br /><textarea id="' + ctrlid + '_param_1" style="width: 98%" cols="50" rows="5" class="txtarea"></textarea>' +
					(tag == 'hide' ? '<br /><label><input type="radio" name="' + ctrlid + '_radio" id="' + ctrlid + '_radio_1" class="pc" checked="checked" />Trả lời mới được xem bài</label><br /><label><input type="radio" name="' + ctrlid + '_radio" id="' + ctrlid + '_radio_2" class="pc" />Chỉ khi điểm của người xem cao hơn</label> <input type="text" size="3" id="' + ctrlid + '_param_2" class="px pxs" /> Chỉ hiển thị khi<br /><br /><label>Ngày hợp lệ:</label> <input type="text" size="3" id="' + ctrlid + '_param_3" class="px pxs" /> <br />Nhãn sẽ tự động hết hạn khi ngày đăng lớn hơn số ngày này' : '');
				break;
			case 'tbl':
				str = '<p class="pbn">Số hàng: <input type="text" id="' + ctrlid + '_param_1" size="2" value="2" class="px" /> &nbsp; Số cột: <input type="text" id="' + ctrlid + '_param_2" size="2" value="2" class="px" /></p><p class="pbn">Chiều rộng bảng: <input type="text" id="' + ctrlid + '_param_3" size="2" value="" class="px" /> &nbsp; Màu nền: <input type="text" id="' + ctrlid + '_param_4" size="2" class="px" onclick="showColorBox(this.id, 2)" /></p><p class="xg2 pbn" style="cursor:pointer" onclick="showDialog($(\'tbltips_msg\').innerHTML, \'notice\', \'Gợi ý\', null, 0)"><img id="tbltips" title="Gợi ý" class="vm" src="' + IMGDIR + '/info_small.gif"> Mẹo viết nhanh</p>';
				str += '<div id="tbltips_msg" style="display: none">“[tr=Màu]” đường viền<br />“[td=Chiều rộng]” chiều rộng cột<br />“[td=Cột, Hàng, Chiều rộng]” Chiều rộng, hàng và cột<br /><br />Ví dụ cách viết nhanh <div class=\'xs0\' style=\'margin:0 5px\'>[table]<br />Name:|Discuz!<br />Version:|X1<br />[/table]</div>Dùng “|” tách cột nếu có “|” dùng“\\|”để ngắt dòng“\\</div>';
				break;
			case 'aud':
				str = '<p class="pbn">Xin vui lòng nhập địa chỉ file nhạc:</p><p class="pbn"><input type="text" id="' + ctrlid + '_param_1" class="px" value="" style="width: 220px;" /></p><p class="xg2 pbn">Định dạng wma mp3 ra rm và các định dạng âm th anh khác<br />Ví dụ: http://server/audio.wma</p>';
				break;
			case 'vid':
				str = '<p class="pbn">Xin vui lòng nhập địa chỉ video:</p><p class="pbn"><input type="text" value="" id="' + ctrlid + '_param_1" style="width: 220px;" class="px" /></p><p class="pbn">Rộng: <input id="' + ctrlid + '_param_2" size="5" value="500" class="px" /> &nbsp; Cao: <input id="' + ctrlid + '_param_3" size="5" value="375" class="px" /></p><p class="xg2 pbn">Hỗ trợ các trang chia sẻ video như Youtube<br />Định dạng wmv avi rmvb mov swf flv và các định dạng video khác<br />Ví dụ: http://server/movie.wmv</p>';
				break;
			case 'fls':
				...
				break;
			case 'beginning':
				str = '<p class="pbn">Điền flash hoặc hình ảnh bắt đầu</p><p class="pbn"><input type="text" id="' + ctrlid + '_param_1" class="px" value="" style="width: 220px;" /></p>';
				str += '<p class="pbn">Bấm vào link:</p><p class="pbn"><input type="text" id="' + ctrlid + '_param_2" class="px" value="" style="width: 220px;" /></p>';
				str += '<p class="pbn">Chiều rộng: <input id="' + ctrlid + '_param_3" size="5" value="" class="px" /> &nbsp; Chiều cao: <input id="' + ctrlid + '_param_4" size="5" value="" class="px" /></p>';
				str += '<p class="pbn">Số giây hiện: <input id="' + ctrlid + '_param_8" size="5" value="" class="px" /></p>';
				str += '<p class="pbn">Hiệu ứng xuất hiện và biến mất: </p><p class="pbn"><input id="' + ctrlid + '_param_7" type="radio" name="effect" checked />Không &nbsp; <input id="' + ctrlid + '_param_5" type="radio" name="effect" />Mờ dần &nbsp; <input id="' + ctrlid + '_param_6" type="radio" name="effect" />Mở và đóng</p>';
				str += '<p class="xg2 pbn">Định dạng swf flv jpg gif png URL<br />Kích thước: 400~1024 300~640<br />Ví dụ: http://server/flash.swf</p>';
				break;
			case 'pasteword':
				stitle = 'Paste nội dung từ file Word';
				...
				break;
			case 'index':
				stitle = 'Tạo chỉ mục bài đăng';
				str = '<p class="pbn">[index]<br />\n\
					[#<span class="xi1">Số trang</span>] <span class="xi1">Tiêu đề</span> &nbsp;&nbsp;<span class="xg1">Nhảy tới trang</span><br />\n\
					<span class="xi1">*</span>[#<span class="xi1">tid,pid</span>] <span class="xi1">Tiêu đề</span> &nbsp;&nbsp;<span class="xg1">Nhảy tới trang</span><br />\n\
					[/index]<br />\n\
					<br />\n\
					<span class="xi1">Số trang</span> &nbsp;&nbsp;<span class="xg1">Sử dụng [page] để phân trang</span><br />\n\
					<span class="xi1">tid,pid</span> &nbsp;&nbsp;<span class="xg1">TID và PID của bài đăng</span><br />\n\
					<span class="xi1">*</span> &nbsp;&nbsp;<span class="xg1">Thụt đầu dòng</span></p>';
				break;
			default:
				...
				if(!str) {
					...
					for(var i = 1; i <= params; i++) {
						if(i != params || !haveSel) {
							str += (promptlang[i - 1] ? promptlang[i - 1] : 'Xin vui lòng nhập ' + i + ' thông số:') + '<br /><input type="text" id="' + ctrlid + '_param_' + i + '" style="width: 98%" value="" class="px" />' + (i < params ? '<br />' : '');
						}
					}
				}
				break;
