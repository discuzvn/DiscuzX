...
function fileQueueError(file, errorCode, message) {
	try {
		if (errorCode === SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED) {
			...
			showDialog("Bạn chọn số lượng file vượt quá giới hạn.\n"+(message === 0 ? "Bạn đã đạt đến giới hạn để tải lên." : "Bạn cũng có thể chọn" + message + " các tập tin"), 'notice', null, null, 0, null, null, null, null, sdCloseTime);//kyehani - discuz.vn
			return;
		}
		...
		switch (errorCode) {
			case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
				...
				this.debug("Error Code: File quá lớn, Tên: " + file.name + ", Dung lượng: " + file.size + ", Nội dung: " + message);
				break;
			case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
				progress.setStatus("Tập tin vô nghĩa.");
				this.debug("Error Code: Tập tin vô nghĩa, Tên: " + file.name + ", Dung lượng: " + file.size + ", Nội dung: " + message);
				break;
			case SWFUpload.QUEUE_ERROR.INVALID_FILETYPE:
				progress.setStatus("Loại tập tin bị cấm.");
				this.debug("Error Code: Lỗi loại file, Tên: " + file.name + ", Dung lượng: " + file.size + ", Nội dung: " + message);
				break;
			case SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED:
				alert("Bạn đã lựa chọn quá nhiều file. " +  (message > 1 ? "Bạn chỉ có thể thêm " +  message + " file" : "Bạn không thể thêm các tập tin."));
				break;
			default:
				if (file !== null) {
					progress.setStatus("Unhandled Error");
				}
				this.debug("Error Code: " + errorCode + ", Tên: " + file.name + ", Dung lượng: " + file.size + ", Nội dung: " + message);
				break;
		}
	} catch (ex) {
        this.debug(ex);
...
function uploadProgress(file, bytesLoaded, bytesTotal) {
	...
	try {
		...
		var progress = new FileProgress(file, this.customSettings.progressTarget);
		progress.setStatus("Tải lên ("+percent+"%)...");
	...
	try {
		...
		if(this.customSettings.uploadSource == 'forum') {
			...
			newTr.appendChild(newTd);
				...
				showDialog('Tải lên hình ảnh không thành công', 'notice', null, null, 0, null, null, null, null, sdCloseTime);
			}
			$(file.id).style.display = 'none';
		} else if(this.customSettings.uploadType == 'blog') {
			var data = eval('('+serverData+')');
