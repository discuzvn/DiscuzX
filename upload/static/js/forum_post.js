...
function checklength(theform) {
	...
	showDialog('Chiều dài ' + mb_strlen(message) + ' ký tự, ' + (postmaxchars != 0 ? 'độ dài: ' + postminchars + ' đến ' + postmaxchars + ' ký tự.' : ''), 'notice', 'Kiểm tra số ký tự');
}
...
function validate(theform) {
	...
	if(in_array($('postsubmit').name, ['topicsubmit', 'editsubmit'])) {
		if(theform.typeid && (theform.typeid.options && theform.typeid.options[theform.typeid.selectedIndex].value == 0) && typerequired) {
			showError('Hãy chọn phân loại chủ đề tương ứng');
			return false;
		}
	...
	if(!disablepostctrl && !sortid && !special && ((postminchars != 0 && mb_strlen(message) < postminchars) || (postmaxchars != 0 && mb_strlen(message) > postmaxchars))) {
		showError('Độ dài của bài viết của bạn không hợp lệ.\n\Độ dài hiện tại: ' + mb_strlen(message) + ' ký tự\nGiới hạn hệ thống: ' + postminchars + ' đến ' + postmaxchars + ' ký tự');
		return false;
	}
	if(UPLOADSTATUS == 0) {
	...
	if(isfirstpost && $('adddynamic') != null && $('adddynamic').checked && $('postsave') != null && isNaN(parseInt($('postsave').value)) && ($('readperm') != null && $('readperm').value || $('price') != null && $('price').value)) {
		if(confirm('Vì bạn đã thiết lập quyền đọc hoặc bán bài đăng, bạn có chắc chắn sẽ phát lại nó cho khán giả của mình không?') == false) {
			return false;
		}
	}
...
function uploadNextAttach() {
	...
	if(arr[4] == 'ban') {
		...
		sizelimit = '(Không quá ' + arr[5] + ' byte)';
	} else if(arr[4] > 0) {
		sizelimit = '(Không quá ' + arr[4] + ' byte)';
	}
...
function uploadAttach(curId, statusid, prefix, sizelimit) {
	...
	$(prefix + 'cpdel_' + nextId).innerHTML = '<img src="' + IMGDIR + '/loading.gif" alt="Đang tải..." />';
	UPLOADSTATUS = 1;
...
function uploadWindowload() {
	...
	if(arr[0] == 'DISCUZUPLOAD' && arr[2] == 0) {
		...
		if(arr[7] == 'ban') {
			...
			sizelimit = '(Không quá ' + arr[8] + ' byte)';
		} else if(arr[7] > 0) {
			sizelimit = '(Không quá ' + arr[7] + ' byte)';
		}
...
function addpolloption() {
	if(curoptions < maxoptions) {
		...
		$('polloption_new').innerHTML = 'Đã đạt tối đa số phiếu' + maxoptions;
	}
}
...
function getreplycredit() {
	...
	if(real_reply_credit > userextcredit) {
		$('replycredit').innerHTML = '<b class="xi1">Tổng số tiền thưởng điểm quá lớn('+real_reply_credit+')</b>';
	} else {
		if(have_replycredit > 0 && real_reply_credit < 0) {
			$('replycredit').innerHTML = "<font class='xi1'>"+Math.abs(real_reply_credit)+" trở lại</font>";
		} else {
