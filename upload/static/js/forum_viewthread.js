		...
		for(pid in aimgcount) {
			...
			for(i = 0;i < aimgcount[pid].length;i++) {
				...
				imagelist += '<div class="pattimg">' +
					'<a id="pattimg_' + aimgcount[pid][i] + '" class="pattimg_zoom" href="javascript:;"' + s + ' onclick="zoom($(\'aimg_' + aimgcount[pid][i] + '\'), attachimggetsrc(\'aimg_' + aimgcount[pid][i] + '\'), 0, 0, ' + (parseInt(showexif) ? 1 : 0) + ')" title="Phóng to">Phóng to</a>' +
...
function setanswer(pid, from){
	if(confirm('Bạn có chắc trả lời chọn là "câu trả lời tốt nhất"?')){
		if(BROWSER.ie) {
			doane(event);
		}
...
function connect_share(connect_share_url, connect_uin) {
	if(parseInt(discuz_uid) <= 0) {
		...
		if(connect_uin) {
			...
			showDialog($('connect_share_unbind').innerHTML, 'info', 'Xin vui lòng liên kết tài khoản');
		}
		return false;
	}
}
...
function connect_get_thread() {
	...
	if ($('postmessage_' + connect_thread_info.post_id)) {
		connect_thread_info.html_content = preg_replace(["'"], ['%27'], encodeURIComponent(preg_replace(['Bài viết của .*? tại .*? chỉnh sửa','&nbsp;','<em onclick="copycode\\(\\$\\(\'code0\'\\)\\);">Sao chép mã</em>'], ['',' ', ''], $('postmessage_' + connect_thread_info.post_id).innerHTML)));
	}
...
function show_threadpage(pid, current, maxpage, ispreview) {
	...
	if(current < maxpage) {
		s += '<a href="javascript:;" onclick="' + clickvalue(current + 1) + '" class="nxt">Tiếp theo</a>';
	}
