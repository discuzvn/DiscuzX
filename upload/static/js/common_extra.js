...
function _showselect(obj, inpid, t, rettype) {
	...
	if(!obj.id) {
		...
		if(!t) {
			...
			s += showselect_row(inpid, 'Tuỳ chọn', -2);
		} else {
			if($(t)) {
				...
				s += showselect_row(inpid, 'Tuỳ chọn', -1);
			} else {
...
function _zoom(obj, zimg, nocover, pn, showexif) {
	...
	if(!menu && !pn) {
		...
		menu.id = menuid;
		if(cover) {
			menu.innerHTML = '<div class="zoominner" id="' + menuid + '_zoomlayer" style="display:none"><p><span class="y"><a id="' + menuid + '_imglink" class="imglink" target="_blank" title="Mở cửa sổ mới">Mở cửa sổ mới</a><a id="' + menuid + '_adjust" href="javascipt:;" class="imgadjust" title="Kích thước thực">Kích thước thực</a>' +
				'<a href="javascript:;" onclick="hideMenu()" class="imgclose" title="Đóng">Đóng</a></span>Lăn chuột để phóng to</p>' +
...
function _setShortcut() {
	$('shortcuttip').onclick = function() {
		var msg = '1. Bấm vào "' + '<a href="javascript:;" class="xi2 xw1" ';
		msg += 'onclick="this.href = \'forum.php?mod=misc&action=shortcut\';this.click();saveUserdata(\'setshortcut\', 1);"';
		msg += '>tạo shortcut</a>' + '", để đưa shortcut ra màn hình Desktop<br />';
		msg += '2. Bấm vào "' + '<a href="forum.php?mod=misc&action=shortcut&type=ico" class="xi2 xw1">';
		msg += 'icon</a>' + '", đề đưa icon ra màn hình Desktop';
		showDialog(msg, 'notice', 'Thêm shortcut ra màn hình');
	};
