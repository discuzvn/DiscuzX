...
if($op == 'post') {
	...
	$_GET['starttime'] = !preg_match("/^(0|\d{4}\-\d{1,2}\-\d{1,2})$/", getgpc('starttime')) ? dgmdate(TIMESTAMP - 86400 * ($_G['adminid'] == 2 ? 13 : ($_G['adminid'] == 3 ? 6 : 60)), 'd-m-Y') : getgpc('starttime');
	$_GET['endtime'] = $_G['adminid'] == 3 || !preg_match("/^(0|\d{4}\-\d{1,2}\-\d{1,2})$/", getgpc('endtime')) ? dgmdate(TIMESTAMP, 'd-m-Y') : getgpc('endtime');
	...
	if($do == 'search' && submitcheck('searchsubmit', 1)) {
		...
		if($_G['adminid'] == 1 && $endtime != dgmdate(TIMESTAMP, 'd-m-Y')) {
			if($endtime != '0') {
