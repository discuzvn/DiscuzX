	...
	make : function (id, child) {
		...
		x.getJSON(this.url+id+'&_makehtml&r='+(+ new Date()), function(ret){
			...
			if(obj.single) {
				...
				progress_bar(cent);
				var str = 'Cần tạo '+obj.count+' '+obj.dom.getAttribute('mktitle')+' files, đã tạo thành công '+obj.makedcount;
				if(cent != 100) {
					child.innerHTML = str+' đang tạo'+current+', hoàn thành '+cent+'%';
				} else {
					child.style.color = 'blue';
