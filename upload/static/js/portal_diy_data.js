...
drag.extend({
	...
	init : function (sampleMode) {
		this.initCommon();
		$('samplepanel').innerHTML = 'Quản lý các dữ liệu mô-đun [<a href="javascript:;" onclick="spaceDiy.cancel();return false;" class="xi2">Đóng</a>]';
	...
	blockForceUpdate : function (e,all) {
		...
		bcontent.innerHTML = '<center>Đang tải...</center>';
		var x = new Ajax();
		x.get('portal.php?mod=portalcp&ac=block&op=getblock&forceupdate=1&inajax=1&bid='+bid+'&tpl='+document.diyform.template.value, function(s) {
			if(s.indexOf('errorhandle_') != -1) {
				...
				runslideshow();
				showDialog('Xin lỗi, bạn không được phép thêm hoặc chỉnh sửa mô-đun', 'alert');
				doane();
			} else {
