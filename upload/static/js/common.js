...
function showDialog(msg, mode, t, func, cover, funccancel, leftmsg, confirmtxt, canceltxt, closetime, locationtime) {
	...
	if(locationtime) {
		leftmsg = locationtime + ' giây tự động chuyển';
		showDialogST = setTimeout(closefunc, locationtime * 1000);
		showconfirm = 0;
	}
	confirmtxt = confirmtxt ? confirmtxt : confirmtxtdefault;
	canceltxt = canceltxt ? canceltxt : 'Hủy bỏ';
...
function showWindow(k, url, mode, cache, menuv) {
	...
	var fetchContent = function() {
		...
		if(parseInt(BROWSER.ie) != 6) {
			loadingst = setTimeout(function() {showDialog('', 'info', '<img src="' + IMGDIR + '/loading.gif"> Đang tải...')}, 500);
		}
	};
...
function setCopy(text, msg) {
	...
	if(success) {
		...
		if(r) {
			...
			showDialog('<div class="c"><div style="width: 200px; text-align: center;">Copy lỗi, hay cho phép quyền copy</div></div>', 'alert');
		}
	} else {
		var msg = '<div class="c"><div style="width: 200px; text-align: center; text-decoration:underline;">Bấm vào đây để copy</div>' +
...
function noticeTitleFlash() {
	if(NOTICETITLE.flashNumber < 5 || NOTICETITLE.flashNumber > 4 && !NOTICETITLE['State']) {
		document.title = (NOTICETITLE['State'] ? '[      ]' : '[Tin mới]') + NOTICETITLE['oldTitle'];
...
function addFavorite(url, title) {
	try {
		...
		try {
			...
			showDialog("Nhấn Ctrl + D để thêm vào Favorite", 'notice');
		}
	}
}
...
function setHomepage(sURL) {
	if(BROWSER.ie){
		...
		showDialog("Đối với các trình duyệt không phải IE, vui lòng đặt trang web này làm trang chủ theo cách thủ công", 'notice');
		doane();
	}
}
...
function toggleBlind(dom) {
	if(dom) {
		if(loadUserdata('is_blindman')) {
			...
			dom.title = 'Bật quyền truy cập phụ trợ';
		} else {
			saveUserdata('is_blindman', '1');
			dom.title = 'Tắt quyền truy cập phụ trợ';
		}
	}
}
