...
if(!($operation)) {
	...
	if(!submitcheck('tasksubmit')) {
		...
		foreach(C::t('common_task')->fetch_all_data() as $task) {
			...
			if($task['starttime'] && $task['endtime']) {
				$task['time'] = dgmdate($task['starttime'], 'd-m-Y H:i').' ~ '.dgmdate($task['endtime'], 'd-m-Y H:i');
			} elseif($task['starttime'] && !$task['endtime']) {
				$task['time'] = dgmdate($task['starttime'], 'd-m-Y H:i').' '.cplang('tasks_online');
			} elseif(!$task['starttime'] && $task['endtime']) {
				$task['time'] = dgmdate($task['endtime'], 'd-m-Y H:i').' '.cplang('tasks_offline');
			} else {
	...
	if(!submitcheck('editsubmit')) {
		...
		showsetting('tasks_add_starttime', 'starttime', $task['starttime'] ? dgmdate($task['starttime'], 'd-m-Y H:i') : '', 'calendar', '', 0, '', 1);
		showsetting('tasks_add_endtime', 'endtime', $task['endtime'] ? dgmdate($task['endtime'], 'd-m-Y H:i') : '', 'calendar', '', 0, '', 1);
		showsetting('tasks_add_periodtype', array('periodtype', array(
			array(0, cplang('tasks_add_periodtype_hour')),
			array(1, cplang('tasks_add_periodtype_day')),
			array(2, cplang('tasks_add_periodtype_week')),
