...
function rewriteoutput($type, $returntype, $host) {
	...
	if($type == 'forum_forumdisplay') {
		...
		$r = array(
			...
			'{page}' => $page ? $page : 1,
			'{forumslug}' => slugify(DB::result_first("SELECT name FROM ".DB::table('forum_forum')." WHERE fid='$fid'")),
		);
		...
		$r = array(
			...
			'{threadslug}' => slugify(DB::result_first("SELECT subject FROM ".DB::table('forum_thread')." WHERE tid='$tid'")),
		);
		...
		$r = array(
			'{id}' => $id,
			'{page}' => $page ? $page : 1,
			'{articleslug}' => slugify(DB::result_first("SELECT title FROM ".DB::table('portal_article_title')." WHERE aid='$id'")),
		);
