		...
		if($member) {
			...
			if($clist) {
				...
				foreach($clist as $crime) {
					showtablerow('', '', array('<a href="home.php?mod=space&uid='.$member['uid'].'">'.$member['username'], $lang[$crime['action']], date('d-m-Y H:i:s', $crime['dateline']), $crime['reason'], '<a href="home.php?mod=space&uid='.$crime['operatorid'].'" target="_blank">'.$crime['operator'].'</a>'));
				}
				echo '</table></td></tr>';
			}
			showtagfooter('tbody');
	...
	if(!$_GET['ipact']) {
		if(!submitcheck('ipbansubmit')) {
			...
			foreach(C::t('common_banned')->fetch_all_order_dateline() as $banned) {
				...
				$banned['dateline'] = dgmdate($banned['dateline'], 'd-m-Y');
				$banned['expiration'] = dgmdate($banned['expiration'], 'd-m-Y');
