<?php

/**
 *      [Discuz!] (C)2001-2099 Comsenz Inc.
 *      This is NOT a freeware, use is subject to license terms
 *
 *      $Id: lang_gift.php 27449 2012-02-01 05:32:35Z zhangguosheng $
 */
if (!defined('IN_DISCUZ')) {
    exit('Access Denied');
}
$lang = array(
    'gift_name' => 'Thẻ Lì Xì',
    'gift_desc' =>
        'Treo phong bao lì xì ở Blog của bạn, khách ghé thăm nhấn vào có thể nhận được phần thưởng.',
    'gift_info' =>
        'Một phong bì mầu đỏ (có thể được chia thành nhiều phần) treo trong Blog của bạn, <br /> Mỗi khách truy cập có thể nhận được một phần tiền đặt trước',
    'gift_succeed' => 'Đặt phong bì thành công',
    'gift_bad_credits_input' => 'Nhập tổng số điểm không chính xác',
    'gift_bad_percredit_input' =>
        'Nhập số điểm cho từng người không chính xác ',
    'gift_bad_credittype_input' => 'Nhập sai quy định tính điểm',
    'gift_credits_out_of_own' => 'Điền số Tiền vượt quá số Tiền bạn có rồi',
    'gift_gc' => 'Treo lại phong bì đỏ',
    'gift_use' => 'Treo phong bì đỏ',
    'gift_receive_gift' => 'Mở lì xì nhận được {percredit} {credittype}',
);
