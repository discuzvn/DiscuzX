	...
	if($count) {
		...
		foreach($cardlist AS $key => $val) {
			showtablerow('', array('class="smallefont"', '', '', '', '', '', '', '', '', '', '', ''), array(
				...
				$val['cleardateline'] ? dgmdate($val['cleardateline'], 'd-m-Y') : cplang('card_make_cleardateline_none'),
...
EOT;
		...
		showsetting('card_make_cleardateline', 'cleardateline', date("d-m-Y", $_G['timestamp']+31536000), 'calendar', '', 0, '');
		...
		foreach(C::t('common_card_log')->fetch_all_by_operation($operation, $start_limit, $perpage) as $result) {
			...
			if($result['operation'] == 1 || $result['operation'] == 2) {
				...
				$showrule = array(
					...
					cplang('card_make_cleardateline').' : '.($result['cardrule_arr']['cleardateline'] ? dgmdate($result['cardrule_arr']['cleardateline'], 'd-m-Y H:i') : cplang('card_make_cleardateline_none')),
				);
