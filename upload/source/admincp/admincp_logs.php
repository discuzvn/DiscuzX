...
if($operation == 'illegal') {
	...
	foreach(C::t('forum_warning')->fetch_all_by_author(($_GET['keyword'] ? explode(',', $_GET['keyword']) : null), $start, $lpp) as $row) {
		showtablerow('', array('class="td28"', 'class=""', '', 'class="td26"', ''), array(
			...
			dgmdate($row['dateline'], 'd-m-Y H:i'),
		));
	}
	...
	if(submitcheck('crimesearch', 1)) {
		...
		if($clist) {
			...
			foreach($clist as $crime) {
				showtablerow('', '', array('<a href="home.php?mod=space&uid='.$crime['uid'].'" target="_blank">'.$crime['username'].'</a>', $lang[$crime['action']], date('d-m-Y H:i:s', $crime['dateline']), $crime['reason'], '<a href="home.php?mod=space&uid='.$crime['operatorid'].'" target="_blank">'.$crime['operator'].'</a>'));
			}
