...
function show_importfile_list($exportlog = array(), $exportziplog = array(), $exportsize = array(), $exportzipsize = array()) {
	...
	foreach($exportlog as $key => $val) {
		$info = $val[1];
		$info['dateline'] = is_int($info['dateline']) ? gmdate('d-m-Y H:i:s', $info['dateline'] + 3600 * 8) : lang('unknown');
...
function show_msg($message, $url_forward = '', $type = 'message', $success = 0) {
	...
	if($type == 'message') {
		...
		echo "$message ...";
		echo "<br /><br /><br /><a href=\"$url_forward\">The browser will automatically jump to the page without manual intervention. Unless your browser does not automatically redirect for a long time, please click here</a>";
