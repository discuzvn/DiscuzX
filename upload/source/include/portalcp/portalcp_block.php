...
if($op == 'block') {
	...
	if(submitcheck('blocksubmit')) {
		...
		$_POST['dateformat'] = in_array($_POST['dateformat'], array('d-m-Y', 'm-d', 'H:i', 'd-m-Y H:i')) ? $_POST['dateformat'] : 'd-m-Y';
	...
	foreach($item['fields'] as $key=>$value) {
		if($theclass['fields'][$key]) {
			switch($theclass['fields'][$key]['datatype']) {
				case 'date':
					$itemfields[$key] = dgmdate($value, 'd-m-Y H:i:s');
					break;
				case 'int':
...
function block_getdateformats($format='') {
	$formats = array('d-m-Y', 'm-d', 'H:i', 'd-m-Y H:i');
	$return = array();
