...
if(!submitcheck('editsubmit')) {
	...
	if($isfirstpost) {
		...
		if($rushreply) {
			...
			$postinfo['rush']['starttimefrom'] = $postinfo['rush']['starttimefrom'] ? dgmdate($postinfo['rush']['starttimefrom'], 'd-m-Y H:i') : '';
			$postinfo['rush']['starttimeto'] = $postinfo['rush']['starttimeto'] ? dgmdate($postinfo['rush']['starttimeto'], 'd-m-Y H:i') : '';
		}
		...
		if($thread['special'] == 1 && ($_G['group']['alloweditpoll'] || $thread['authorid'] == $_G['uid'])) {
			...
			$activity['starttimefrom'] = dgmdate($activity['starttimefrom'], 'd-m-Y H:i');
			$activity['starttimeto'] = $activity['starttimeto'] ? dgmdate($activity['starttimeto'], 'd-m-Y H:i') : '';
			$activity['expiration'] = $activity['expiration'] ? dgmdate($activity['expiration'], 'd-m-Y H:i') : '';
			...
			$debate['endtime'] = $debate['endtime'] ? dgmdate($debate['endtime'], 'd-m-Y H:i') : '';
		}
	...
	if($thread['special'] == 2 && ($thread['authorid'] == $_G['uid'] && $_G['group']['allowposttrade'] || $_G['group']['allowedittrade'])) {
		...
		if($trade) {
			$trade['expiration'] = $trade['expiration'] ? date('d-m-Y', $trade['expiration']) : '';
			$trade['costprice'] = $trade['costprice'] > 0 ? $trade['costprice'] : '';
			$trade['message'] = dhtmlspecialchars($trade['message']);
			$expiration_7days = date('d-m-Y', TIMESTAMP + 86400 * 7);
			$expiration_14days = date('d-m-Y', TIMESTAMP + 86400 * 14);
			$expiration_month = date('d-m-Y', mktime(0, 0, 0, date('m')+1, date('d'), date('Y')));
			$expiration_3months = date('d-m-Y', mktime(0, 0, 0, date('m')+3, date('d'), date('Y')));
			$expiration_halfyear = date('d-m-Y', mktime(0, 0, 0, date('m')+6, date('d'), date('Y')));
			$expiration_year = date('d-m-Y', mktime(0, 0, 0, date('m'), date('d'), date('Y')+1));
		} else {
			$special = 0;
			$trade = array();
		}
